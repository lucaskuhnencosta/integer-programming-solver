...
def build_ip_model(parsed_data,best_A,clique_constraints=None,ip_filename="modelIP.lp"):
    """
    Builds and returns an IP model of the problem with optional clique constraints.
    ...
    # Decision variables
    X = model.addVars(num_orders, vtype=GRB.BINARY, name="X")
    Y = model.addVars(num_aisles, vtype=GRB.BINARY, name="Y")
    ...
    #model.write(ip_filename)
    model.setParam('OutputFlag', 0)


    print(f"     IP model successfully built with {len(clique_constraints or [])} clique constraints.")
    return model, X, Y
